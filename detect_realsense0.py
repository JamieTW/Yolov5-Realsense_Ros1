...
def run(weights=ROOT / 'yolov5m.pt',  # model.pt path(s)
    ...
    # Innitialize prediction publisher
    pred_pub = rospy.Publisher(
        'output_BBoxes', BoundingBoxes, queue_size=10
    )
    # Innitialize image publisher
    image_pub = rospy.Publisher(
        'output_preimages', Image, queue_size=10
    )
    ...
    while(True):
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    bounding_box.Class = names[c]
                    bounding_box.probability = Ztarget
                    ...
                    # Annotate the image
                    if image_pub or True:  # Add bbox to image
                          # integer class
        ...
        image_pub.publish(bridge.cv2_to_imgmsg(im0, "bgr8"))
